# Voice Input by Ricky (portable, no admin)

_A lightweight Windows voice-typing tool: hold Right Alt, speak, and the text lands at your cursor. No installer, no admin rights, no system input-method changes._
...
## Known limitation: very long dictation

Utterances longer than roughly 40 seconds are **not reliable yet** in this build. The
vendor engine's `--test-sami` interface reads a *complete* WAV file: its read rate varies
(0.6x-2.5x real time depending on whether the bytes are speech or silence), and reaching the
physical end of the file ends the session immediately. Because of that:

* real-time streaming into the file can lose audio (measured: the reader outruns the writer),
* keeping the file tight makes the session end early,
* so the current build caps the usable utterance at about 40 s.

Short and medium utterances (up to ~40 s, any mix of Mandarin / Cantonese / English) work
normally. Fixing long dictation properly needs the resident-engine route (Plan B): keep one
engine process alive, let it capture the microphone itself and drive it through its local
pipe, which removes the file from the loop entirely. See
`outputs/Plan B - resident engine prep.md`.
